    ...
    public static String deviceToken(Context context) {
        ...
        try {
            ...
            if (!file.exists()) {
                ...
                deviceToken.token = Des.encode(UUID.randomUUID().toString());
                SerializableUtils.output(file, deviceToken);
            } else {
                deviceToken = SerializableUtils.load(file, DeviceToken.class);
            }
            ...
            e.printStackTrace();
            try {
                return Des.encode(uniqueId(context));
            } catch (Exception e1) {
                e1.printStackTrace();
            }
        }
